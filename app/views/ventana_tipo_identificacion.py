...
from controllers import controlador_tipo_identificaciones
...
class VentanaTipoIdentificacion(tk.Frame):
    def __init__(self, parent):
        self.tipo_id_model = controlador_tipo_identificaciones
        ...
        titulo = tk.Label(frame_titulo, text="Gestión de Tipos de Identificación", 
        ...
        self.cargar_tipos_identificacion()

        boton_frame = tk.Frame(panel_principal, bg=COLOR_PANEL)
        boton_frame.pack(pady=(5, 15))

        self._crear_boton(boton_frame, "Guardar Tipo", self.guardar_tipo_identificacion)
        self._crear_boton(boton_frame, "Eliminar Tipo", self.eliminar_tipo_identificacion)
        self._crear_boton(boton_frame, "Actualizar Tipo", self.actualizar_tipo_identificacion)
    ...
    def cargar_tipos_identificacion(self):
        for fila in self.tabla.get_children():
            self.tabla.delete(fila)
        datos = self.tipo_id_model.mostrar_tipos_identificacion()
        if datos:
    ...
    def guardar_tipo_identificacion(self):
        self.tipo_id_model.guardar_tipo_identificacion(self.entry_descripcion.get())
        self.limpiar()

    def eliminar_tipo_identificacion(self):
        self.tipo_id_model.eliminar_tipo_identificacion(self.id_seleccionado)
        self.limpiar()

    def actualizar_tipo_identificacion(self):
        self.tipo_id_model.actualizar_tipo_identificacion(self.id_seleccionado, self.entry_descripcion.get())
        self.limpiar()
